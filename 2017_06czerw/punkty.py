...
if __name__ == '__main__':
    ...
    z(1)
    print(len([ 1 for x, y in pary if pierw[int(x)] and pierw[int(y)]]))

    z(2)
    print(len([1 for x, y in pary if set(x) == set(y) ]))

    z(3)
    pary = [(int(x), int(y)) for x, y in pary]

    maks_distance, a, b = 0, None, None
    for xa, ya in pary:
        for xb, yb in pary:
    ...
    print('A', a, 'B', b, 'Dist', round(maks_distance))
